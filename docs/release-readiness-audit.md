...
| --- | --- | --- |
| Feels like part of existing OpenClaw/Codex setup, not a separate worse tool | `clawdeck setup`, `clawdeck adopt`, `clawdeck handoff`, `templates/CLAWDECK.md`, bounded `AGENTS.md` pointer | `src/setup.js` gives a one-command first-run path; `src/adopt.js` detects an existing OpenClaw workspace, overlays missing files, preserves existing `AGENTS.md`, and adds a Clawdeck pointer block. `src/handoff.js` prints a Codex Mac app brief from the same workspace. |
...
| First run has one obvious path | `clawdeck setup --yes` | `src/setup.js` adopts, applies with backup, runs the drill, prints missing model pull commands, and leaves one next action while keeping writes gated behind `--yes`. |
...
| Audit behaves naturally after setup | configured workspace fallback | If `clawdeck audit` is run from a non-workspace folder after setup, it follows `agents.defaults.workspace` instead of scoring the wrong directory. |
| Shareable public surface exists | HTML/Markdown/JSON/SVG audit outputs, launch kit, release notes | `clawdeck audit` renders report artifacts; `docs/launch-kit.md` and `docs/release-notes/v0.1.3.md` are launch-ready. |
...
| Public release path is known | `docs/github-release-runbook.md` | Runbook covers the already-public GitHub repo, `v0.1.3` tag/release, scoped npm publish, and proof commands. |
...
- `npm pack --dry-run`
- `npm run verify`
- packed tarball first-run flow: `setup --yes -> smoke/handoff/audit`
...
- temp existing-workspace flow: `setup --yes -> drill -> smoke`, plus manual `adopt -> apply -> drill`
...
- The public GitHub repo and `v0.1.2` npm package already exist; the CLI edge-case polish should ship as `v0.1.3` instead of mutating the published package.
- The scoped npm `v0.1.3` package still requires an approved `npm publish --access public`.
...
Public release still requires fresh exact approval because it pushes commits/tags, edits repo metadata, creates a GitHub release, and publishes a scoped npm package.
